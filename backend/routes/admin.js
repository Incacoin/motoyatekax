...
router.post("/admin/drivers", checkAdminPin, (req, res) => {
  const { name, phone, vehicle, tipo, acceptedLegal, photo, signature, vehicleType, grupo } = req.body;
  if (!name || !phone) {
  ...
  const result = db
    .prepare(
      "INSERT INTO drivers (name, phone, vehicle, pin, tipo, accepted_legal_at, accepted_legal_version, photo, signature, vehicle_type, grupo) VALUES (?, ?, ?, ?, ?, datetime('now'), ?, ?, ?, ?, ?)"
    )
    .run(name, phone, vehicle || null, pin, tipo === "formal" ? "formal" : "informal", AVISO_LEGAL_VERSION, photo || null, signature || null, vehicleType === "taxi" ? "taxi" : "moto", grupo || null);

  const driver = db
    .prepare("SELECT id, name, phone, vehicle, pin, status, tipo, photo, signature, vehicle_type, grupo FROM drivers WHERE id = ?")
  ...
  const drivers = db
    .prepare(
      `SELECT d.id, d.name, d.phone, d.vehicle, d.pin, d.status, d.last_seen, d.paid_until, d.vouched_by, d.vouched_at,
              d.tipo, d.photo, d.signature, d.vehicle_type, d.cancel_count, d.cooldown_until, d.grupo, d.created_at,
...
router.post("/admin/chofer-solicitudes/list", checkAdminPin, (req, res) => {
  const apps = db
    .prepare(
      "SELECT id, name, phone, photo, signature, status, created_at, accepted_legal_at, accepted_legal_version, vehicle_type FROM driver_applications WHERE status = 'pendiente' ORDER BY created_at DESC"
    )
    .all();
  res.json(apps);
});
